...
def main():
    args = docopt(__doc__)
    t = VeryPrettyTable(field_names=('msg', 'char', 'coefficient', 'decrypted'))
    t.align = 'l'
    for m in open(args['MSG_FILE']):
        result = detect_msg(m.strip())
        if not result:
            continue
        first = result[0]
        t.add_row((m, repr(first[0]), first[1], repr(first[2])))
    print(t.get_string())
